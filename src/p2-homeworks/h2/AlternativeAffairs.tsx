...
const getBooks = async (url: string) => {
  ...
  const booksInfo = await res.json();
    let books = booksInfo.map((el: responseType, i:number) => ({ name: el.name, publisher: el.publisher, id: i }));
    return books
};
...
export default class AlternativeAffairs extends Component {
  ...
  onFiltered = (arr: Array<any>, filter: string) => {
    switch (filter) {
      ...
      case 'Tor Publishing': {
        return arr.filter((el: responseType) => el.publisher === "Tor Fantasy" || el.publisher === "Tor Books")
      }
        break;
      default: 
        return arr
  ...
  render() {
    ...
    let isDataLoaded = bookList ? this.renderBookList(filteredBooks) : "...loaded";
